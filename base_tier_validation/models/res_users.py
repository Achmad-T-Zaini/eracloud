...
class Users(models.Model):
    ...
    def review_user_count(self):
        ...
        for review_group in review_groups:
            ...
            if reviews:
                # * added "x.state in self.env[model]._state_from" in filtered
                # * to exclude record which state is not in _state_from of validation
                records = (
                    self.env[model]
                    .with_user(self.env.user)
                    .search([("id", "in", reviews.mapped("res_id"))])
                    .filtered(lambda x: not x.rejected and x.can_review
                        and x.state in self.env[model]._state_from)
                )
                # if len(records):
                for rec in records:
