...
class TierValidation(models.AbstractModel):
    ...
    def _get_under_validation_exceptions(self):
        ...
        # * add 'message_main_attachment_id'
    ...
    def write(self, vals):
        for rec in self:
            ...
            if ( rec._name != 'crm.lead' and 
                ...
                and not rec.validated
            ):
                raise ValidationError(_("The operation is under validation.%s")%(rec._name))
            elif ( rec._name == 'crm.lead' and 
                rec.review_ids
                and getattr(rec, self._state_field) in self._state_from
                and not vals.get(self._state_field)
                in (self._state_to + [self._cancel_state])
