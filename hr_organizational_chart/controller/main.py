...
class EmployeeChart(http.Controller):
        ...
        if child_ids:
            ...
            for child in child_ids:
                child_table = """<td colspan='""" + str(2) + """'>
                    <table><tr><td><div>"""
                okr_data = request.env['okr.node'].search([('employee_id','=',child.id)],order='id',limit=1)
                okr_id = 0
                if okr_data:
                    okr_id = okr_data.id
                    okr_progress = '{0:.2f}'.format(okr_data.progress)
                    if okr_data.result=='achieved':
                        okr ="""<a><div id='""" + str(okr_id) + """' class='okr_name' style='color: green;'><strong>OKR """ + okr_progress + """</strong></div></a>"""
                    elif okr_data.progress>50:
                        okr ="""<a><div id='""" + str(okr_id) + """' class='okr_name' style='color: yellow;'><strong>OKR """ + okr_progress + """</strong></div></a>"""
                    else:
                        okr ="""<a><div id='""" + str(okr_id) + """' class='okr_name' style='color: red;'><strong>OKR """ + okr_progress + """</strong></div></a>"""
                else:
                    okr=''
                view = str(okr) + """<div id='""" + str(child.id) + """' class='o_level_1'><a>
    ...
    def get_parent_child(self, **post):
        if post:
            ...
            okr_data = request.env['okr.node'].search([('employee_id','=',emp.id)],order='id',limit=1)
            okr_id = 0
            if okr_data:
                okr_id = okr_data.id
                okr_progress = '{0:.2f}'.format(okr_data.progress)
            ...
            view = str(okr) + """ <div id="parent" class='o_chart_head'><a>
